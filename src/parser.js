      ...
      if (this.packetStartFound) {
        ...
        switch (this.state) {
          ...
          case PACKET_CRCH: {
            ...
            this.push(
              Buffer.from([this.packet.stx, this.packet.seqSlaveId, this.packet.length, ...this.packet.data, this.packet.crcl, this.packet.crch])
            )
            if (byte === 0x7f) increment = 2 // Fix for byte stuffing
            // } else {
